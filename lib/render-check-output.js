...
export function renderCheckSuccess(source_file_count, output_mode) {
  ...
  return `${ansi.green('Check passed.')}\n${ansi.dim(`Scanned ${source_file_count} ${pluralize('file', source_file_count)}. Found 0 errors.`)}\n`;
}
...
function formatRichDiagnosticGroup(
  ...
  return `${ansi.cyan(formatDiagnosticGroupHeader(diagnostic_group.path))}\n${diagnostic_group.diagnostics
    .map((diagnostic) =>
...
function formatRichDiagnosticRow(diagnostic, diagnostic_prefix_width, ansi) {
  ...
  const diagnostic_prefix = formatDiagnosticPrefix(diagnostic);
  return `  ${ansi.dim(location)}  ${formatRichDiagnosticLevel(diagnostic.level, ansi)}  ${diagnostic.message}${createDiagnosticCodePadding(diagnostic_prefix, diagnostic_prefix_width)}  ${ansi.gray(diagnostic.code)}`;
}
