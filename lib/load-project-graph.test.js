...
it('loads standalone YAML directives into the graph with fallback titles', async () => {
  ...
  expect(
    project_graph_result.graph.nodes['task:docs/tasks/example.yaml'],
  ).toEqual(
    expect.objectContaining({
      $class: 'task',
      $path: 'docs/tasks/example.yaml',
      id: 'task:docs/tasks/example.yaml',
      path: 'docs/tasks/example.yaml',
      status: 'active',
      summary: 'YAML task',
      title: 'example.yaml',
  ...
  expect(project_graph_result.graph.edges).toEqual([
    {
      from: 'task:docs/tasks/example.yaml',
      id: 'edge:1',
      origin: {
        column: 5,
        line: 5,
      ...
      relation: 'tracked_in',
      to: 'doc:docs/plans/v0/example.md',
    },
    {
      from: 'task:docs/tasks/example.yaml',
      id: 'edge:2',
