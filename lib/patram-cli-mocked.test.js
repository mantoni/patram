...
const mocks = vi.hoisted(() => ({
  ...
  create_refs_output_view_mock: vi.fn((refs_output, command_options) => ({
    command_options,
    refs_output,
  })),
...
vi.mock('./inspect-reverse-references.js', () => ({
...
afterEach(() => {
  ...
  mocks.create_refs_output_view_mock.mockImplementation(
    (refs_output, command_options) => ({
      command_options,
      refs_output,
    }),
  );
...
it('loads, filters, and renders refs output and reports invalid where diagnostics', async () => {
  const io_context = createIoContext();
  ...
  expect(mocks.create_refs_output_view_mock).toHaveBeenCalled();
  ...
  mocks.inspect_reverse_references_mock.mockReturnValueOnce({
    diagnostics: [createDiagnostic('bad where')],
    incoming: {},
    node: {
      $class: 'document',
