...
it('renders semantic query output with ids and defining paths', () => {
  const output_view = createOutputView('query', [
    {
      id: 'command:query',
      kind: 'command',
...
it('renders rich semantic query output with the same text layout as plain', async () => {
  const output_view = createOutputView('query', [
    {
      id: 'command:query',
      kind: 'command',
  ...
  expect(stripAnsi(rich_output)).toBe(plain_output);
  expect(rich_output).toContain(ansis.green('command command:query'));
});
