...
function formatRichNodeItem(output_item, ansi, render_options) {
  ...
  if (summary_row) {
    output_lines.push(formatRichSummaryRow(summary_row, '  ', ansi));
  }
...
/**
 * @param {string} summary_row
 * @param {string} indent
 * @param {Ansis} ansi
 * @returns {string}
 */
function formatRichSummaryRow(summary_row, indent, ansi) {
  const summary_label = 'summary:';

  if (!summary_row.startsWith(`${summary_label} `)) {
    return `${indent}${summary_row}`;
  }

  return `${indent}${ansi.gray(summary_label)} ${summary_row.slice(summary_label.length + 1)}`;
}
