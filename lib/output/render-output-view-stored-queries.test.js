...
it('wraps long stored query terms with a hanging indent', () => {
  ...
  expect(renderPlainOutput(output_view)).toBe(
    'plans-without-decisions  $class=plan and status=active and $path^=docs/plans/ and title~Output\n' +
...
it('renders traversal aggregate stored query terms', () => {
  ...
  expect(renderPlainOutput(output_view)).toBe(
    'done  $class=decision\n' +
      '      and none(in:decided_by, $class=task and status not in [done, dropped, superseded])\n',
  );
});
...
it('renders rich stored query boolean keywords in gray', async () => {
  ...
  expect(rich_output).toContain(COLOR_ANSI.green('plans-without-decisions'));
  expect(rich_output).toContain(COLOR_ANSI.gray('='));
  expect(rich_output).toContain(COLOR_ANSI.gray('and'));
  expect(rich_output).toContain(COLOR_ANSI.gray('or'));
  expect(rich_output).toContain(COLOR_ANSI.gray('not'));
  expect(rich_output).not.toContain(COLOR_ANSI.yellow('and'));
});

it('renders an empty stored query list as an empty string', () => {
  expect(layoutStoredQueries([])).toEqual([]);
  expect(
    renderPlainOutput({
      command: 'queries',
      hints: [],
...
it('falls back to literal token wrapping for invalid stored queries', () => {
  expect(
    ...
    [
      { kind: 'name', text: 'broken' },
      { kind: 'plain', text: '  ' },
      { kind: 'literal', text: '(' },
    ],
    [
      { kind: 'name', text: 'blank ' },
      { kind: 'plain', text: '  ' },
      { kind: 'literal', text: '   ' },
    ],
  ]);
