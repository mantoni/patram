...
it('renders optional stored query descriptions with surrounding blank lines', () => {
  const output_view = createOutputView('queries', [
    {
      name: 'active-plans',
      where: '$class=plan and status=active',
...
it('renders rich stored query descriptions in gray', async () => {
  const description_indent = `${' '.repeat('active-plans'.length + 2)}Show active plans.`;
  const output_view = createOutputView('queries', [
    {
      name: 'active-plans',
      where: '$class=plan and status=active',
  ...
  expect(rich_output).toContain(colorAnsi.gray(description_indent));
});
