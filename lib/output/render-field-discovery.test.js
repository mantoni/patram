...
it('limits text evidence output to 5 rows and summarizes the remainder', () => {
  expect(
    renderFieldDiscovery(createOverflowDiscoveryResult(), {
      color_enabled: false,
      renderer_name: 'plain',
    }),
  ).toContain('    2 more ...\n');
...
/**
 * @returns {FieldDiscoveryResult}
 */
function createOverflowDiscoveryResult() {
  return {
    fields: [
      {
        ...DISCOVERY_RESULT.fields[0],
        evidence_references: [
          createEvidenceReference('docs/tasks/alpha.md', 1, 'alpha'),
          createEvidenceReference('docs/tasks/bravo.md', 2, 'bravo'),
          createEvidenceReference('docs/tasks/charlie.md', 3, 'charlie'),
          createEvidenceReference('docs/tasks/delta.md', 4, 'delta'),
          createEvidenceReference('docs/tasks/echo.md', 5, 'echo'),
          createEvidenceReference('docs/tasks/foxtrot.md', 6, 'foxtrot'),
          createEvidenceReference('docs/tasks/golf.md', 7, 'golf'),
        ],
      },
    ],
    summary: {
 ...
 * @param {number} line
 * @param {string} value
 * @returns {{ column: number, line: number, path: string, value: string }}
 */
function createEvidenceReference(path, line, value) {
  return {
    column: 1,
    line,
    path,
