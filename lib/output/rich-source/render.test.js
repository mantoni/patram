...
it('renders visible markdown directive references as rich footnotes', async () => {
  const rich_source = await renderRichSource(
    createShowOutput({
      items: [createDirectiveResolvedLink(3)],
      path: 'docs/release-flow.md',
      source: createDirectiveFootnoteSource(),
    }),
    {
      color_enabled: false,
      color_mode: 'never',
  ...
  expect(rich_source).toBe(createDirectiveFootnoteOutput());
});

it('renders markdown directive list-item references as rich inline summaries', async () => {
  const rich_source = await renderRichSource(
    createShowOutput({
      items: [createDirectiveResolvedLink(3)],
      path: 'docs/check-command.md',
  ...
  expect(rich_source).toBe(createDirectiveListOutput());
});

it('ignores markdown directive references that point past the source text', async () => {
  const rich_source = await renderRichSource(
    createShowOutput({
      items: [createDirectiveResolvedLink(99)],
      path: 'docs/release-flow.md',
      source: createDirectiveFootnoteSource(),
    }),
    {
  ...
  expect(rich_source).toBe(createDirectiveFootnoteSource());
});

it('ignores markdown directive references whose raw target is not found', async () => {
  const rich_source = await renderRichSource(
    createShowOutput({
      items: [createDirectiveResolvedLink(3, 'docs/roadmap/missing.md')],
      path: 'docs/release-flow.md',
      source: createDirectiveFootnoteSource(),
    }),
    {
...
it('styles rich prose footnote identity headers with the shared green accent', async () => {
  const rich_source = await renderRichSource(
    createShowOutput({
      items: [
        {
  ...
  expect(rich_source).toContain(colorAnsi.gray('[^1]'));
  expect(rich_source).toContain(colorAnsi.green('document docs/guide.md'));
});

it('styles rich list annotation identity headers with the shared green accent', async () => {
  const rich_source = await renderRichSource(
    createShowOutput({
      items: [
        {
...
/**
 * @param {{ items: Array<{ kind: 'resolved_link', label: string, reference: number, source?: { claim_type: 'directive', column: number, line: number, parser?: string, raw_target: string }, target: { kind?: string, path: string, title: string } }>, path: string, source: string }} values
...
/**
 * @param {{ kind: 'resolved_link', label: string, reference: number, source?: { claim_type: 'directive', column: number, line: number, parser?: string, raw_target: string }, target: { kind?: string, path: string, title: string } }} item
...
function createResolvedLinkItem(item) {
  ...
  const resolved_link = {
    ...
    target: {
      ...
      kind: item.target.kind ?? 'document',
  ...
  if (item.source) {
    resolved_link.source = item.source;
  }
 ...
 * @returns {{ kind: 'resolved_link', label: string, reference: number, source: { claim_type: 'directive', column: number, line: number, parser: 'markdown', raw_target: string }, target: { kind: 'roadmap', path: string, title: string } }}
 */
function createDirectiveResolvedLink(
  line_number,
...
function createDirectiveFootnoteOutput() {
  return [
    '# Release Flow',
    '',
    ...
    'More text.',
    '',
    '[^1] roadmap docs/roadmap/v0-dogfood.md',
    '    V0 Dogfood',
  ].join('\n');
}

function createDirectiveFootnoteSource() {
  return [
    '# Release Flow',
    '',
...
function createDirectiveListOutput() {
  return [
    '# Check Command Proposal',
    '',
    '• tracked_in: docs/roadmap/v0-dogfood.md',
    '  -> roadmap docs/roadmap/v0-dogfood.md',
    '     V0 Dogfood',
    '• next step',
  ].join('\n');
}
