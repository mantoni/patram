  ...
  for (const annotation of annotations) {
    ...
    rendered_parts.push(
      ...formatInlineResolvedLinkBlock(
        annotation,
        `${annotation_indent}-> ${formatInlineListResolvedLinkHeader(annotation)}`,
        {
 ...
 * @returns {string}
 */
function formatInlineListResolvedLinkHeader(resolved_link) {
  ...
  return `${resolved_link.target.kind} ${resolved_link.target.path ?? resolved_link.target.id}${metadata_label}`;
}
...
function formatFootnoteResolvedLinkHeader(resolved_link) {
  ...
  return `[^${resolved_link.reference}] ${resolved_link.target.kind} ${resolved_link.target.path ?? resolved_link.target.id}${metadata_label}`;
}
