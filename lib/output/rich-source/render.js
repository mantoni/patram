 ...
 * @typedef {{
 *   ansi: Ansis,
 *   current_section_level: number | null,
 *   next_reference: number,
 *   next_top_level_list: number,
 *   pending_section_footnotes: OutputResolvedLinkItem[],
...
async function renderRichMarkdownSource(output_view, ansi) {
  const markdown_tree = parseAST(createRichMarkdownSourceText(output_view));
  /** @type {string[]} */
  const rendered_blocks = [];
  /** @type {RichSourceRenderState} */
  const render_state = {
    ansi,
    current_section_level: null,
    next_reference: 1,
    next_top_level_list: 0,
    pending_section_footnotes: [],
  ...
  for (const node of markdown_tree.nodes) {
    if (isHeadingNode(node)) {
      const pending_footnotes = flushPendingSectionFootnotes(
        render_state,
        getHeadingLevel(getElementTag(node)),
      );

      if (pending_footnotes.length > 0) {
        rendered_blocks.push(pending_footnotes);
      }
    }
    ...
    if (isHeadingNode(node)) {
      render_state.current_section_level = getHeadingLevel(getElementTag(node));
    }
  }

  const trailing_footnotes = flushPendingSectionFootnotes(render_state);

  if (trailing_footnotes.length > 0) {
    rendered_blocks.push(trailing_footnotes);
  }
 ...
 * @returns {string}
 */
function createRichMarkdownSourceText(output_view) {
  const directive_links = output_view.items.filter(isMarkdownDirectiveLink);

  if (directive_links.length === 0) {
    return output_view.source;
  }

  const source_lines = output_view.source.split('\n');

  for (const directive_link of directive_links.sort(
    compareDescendingDirectiveLinks,
  )) {
    const source_details = directive_link.source;

    if (!source_details) {
      continue;
    }

    const source_line = source_lines[source_details.line - 1];

    if (source_line === undefined) {
      continue;
    }

    source_lines[source_details.line - 1] = linkifyDirectiveTarget(
      source_line,
      source_details.column,
      source_details.raw_target,
    );
  }
 ...
 * @param {RichSourceRenderState} render_state
 * @param {{ annotations?: OutputResolvedLinkItem[], kind: 'list' | 'prose' }} inline_context
 * @returns {string}
 */
function renderInlineNodes(nodes, render_state, inline_context) {
  /** @type {string[]} */
  ...
  for (const node of nodes) {
    ...
    if (node_tag === 'strong') {
      rendered_chunks.push(
        render_state.ansi.bold(
          renderInlineNodes(node_children, render_state, inline_context),
        ),
      );
      continue;
    }
    ...
    if (node_tag === 'a') {
      rendered_chunks.push(renderLinkNode(node, render_state, inline_context));
      continue;
    }
...
/**
 * @param {OutputResolvedLinkItem} resolved_link
 * @returns {boolean}
 */
function isMarkdownDirectiveLink(resolved_link) {
  return (
    resolved_link.source?.claim_type === 'directive' &&
    resolved_link.source.parser === 'markdown'
  );
}

/**
 * @param {OutputResolvedLinkItem} left_link
 * @param {OutputResolvedLinkItem} right_link
 * @returns {number}
 */
function compareDescendingDirectiveLinks(left_link, right_link) {
  const left_source = left_link.source;
  const right_source = right_link.source;

  if (!left_source || !right_source) {
    return 0;
  }

  if (left_source.line !== right_source.line) {
    return right_source.line - left_source.line;
  }

  return right_source.column - left_source.column;
}
 ...
 * @returns {string}
 */
function linkifyDirectiveTarget(source_line, source_column, raw_target) {
  const target_index = source_line.indexOf(raw_target, source_column - 1);

  if (target_index < 0) {
    return source_line;
  }

  return `${source_line.slice(0, target_index)}[${raw_target}](${raw_target})${source_line.slice(target_index + raw_target.length)}`;
}
...
async function renderListItem(
  ...
  for (const annotation of annotations) {
    const annotation_indent = `${'  '.repeat(indent_level + 1)}`;

    rendered_parts.push(
      ...formatInlineResolvedLinkBlock(
        annotation,
        `${annotation_indent}${formatRichInlineListResolvedLinkHeader(annotation, render_state.ansi)}`,
        {
          ansi: render_state.ansi,
          body_indent: `${annotation_indent}   `,
        },
      ),
    );
  }
 ...
 * @returns {string}
 */
function renderLinkNode(node, render_state, inline_context) {
  const node_children = getElementChildren(node);
  const node_props = getElementProps(node);
  const link_text = renderInlineNodes(
    node_children,
    render_state,
    inline_context,
  );
  ...
  const resolved_link = consumeResolvedLink(render_state);

  if (!resolved_link) {
    return render_state.ansi.underline(link_text);
  }

  if (inline_context.kind === 'list') {
    inline_context.annotations?.push(resolved_link);
    return render_state.ansi.underline(link_text);
  }

  render_state.pending_section_footnotes.push(resolved_link);
...
/**
 * @param {RichSourceRenderState} render_state
 * @returns {OutputResolvedLinkItem | undefined}
 */
function consumeResolvedLink(render_state) {
  const resolved_link =
...
/**
 * @param {RichSourceRenderState} render_state
 * @param {number | undefined} [next_heading_level]
 * @returns {string}
 */
function flushPendingSectionFootnotes(render_state, next_heading_level) {
  if (render_state.pending_section_footnotes.length === 0) {
    return '';
  }

  if (
    next_heading_level !== undefined &&
    render_state.current_section_level !== null &&
    next_heading_level > render_state.current_section_level
  ) {
    return '';
  }

  const rendered_footnotes = render_state.pending_section_footnotes
    .map((resolved_link) =>
      formatInlineResolvedLinkBlock(
        resolved_link,
        formatRichFootnoteResolvedLinkHeader(resolved_link, render_state.ansi),
        {
          ansi: render_state.ansi,
          body_indent: '    ',
        },
  ...
  render_state.pending_section_footnotes = [];

  return rendered_footnotes;
}

/**
 * @param {OutputResolvedLinkItem} resolved_link
 * @param {string} header_line
 * @param {{ ansi: Ansis, body_indent: string }} render_options
 * @returns {string[]}
 */
function formatInlineResolvedLinkBlock(
  resolved_link,
  header_line,
  render_options,
) {
  ...
  const output_lines = [
    header_line,
    `${render_options.body_indent}${resolved_link.target.title}`,
  ];

  if (resolved_link.target.description) {
    output_lines.push(
      `${render_options.body_indent}${render_options.ansi.gray(resolved_link.target.description)}`,
    );
  }
 ...
 * @returns {string}
 */
function formatRichInlineListResolvedLinkHeader(resolved_link, ansi) {
  return formatRichInlineResolvedLinkHeader(resolved_link, '->', ansi);
}
 ...
 * @returns {string}
 */
function formatRichFootnoteResolvedLinkHeader(resolved_link, ansi) {
  return formatRichInlineResolvedLinkHeader(
    resolved_link,
    `[^${resolved_link.reference}]`,
    ansi,
  );
}
 ...
 * @returns {string}
 */
function formatRichInlineResolvedLinkHeader(resolved_link, marker, ansi) {
  const visible_fields = resolved_link.target.visible_fields;
  const metadata_label =
    visible_fields.length === 0
      ? ''
      : formatRichInlineResolvedLinkMetadataLabel(
          `(${visible_fields
            .map((field) => formatVisibleField(field.name, field.value))
            .join(', ')})`,
          ansi,
        );
  const target_identity = `${resolved_link.target.kind} ${resolved_link.target.path ?? resolved_link.target.id}`;

  return `${ansi.gray(marker)} ${ansi.green(target_identity)}${metadata_label.length > 0 ? `  ${metadata_label}` : ''}`;
}
 ...
 * @returns {string}
 */
function formatVisibleField(field_name, field_value) {
  return `${field_name}=${Array.isArray(field_value) ? field_value.join(', ') : field_value}`;
}
 ...
 * @returns {string}
 */
function formatRichInlineResolvedLinkMetadataLabel(metadata_label, ansi) {
 ...
 */
function collectListItemParts(node, render_state, inline_context) {
  const node_children = getElementChildren(node);
  /** @type {ComarkElement[]} */
  const block_nodes = [];
  /** @type {string | null} */
  ...
  for (
    ...
    if (typeof child === 'string') {
      lead_text = appendLeadText(
        lead_text,
        renderInlineNodes([child], render_state, inline_context),
      );
      continue;
    }
...
/**
 * @param {ComarkNode} node
 * @returns {node is ComarkElement}
 */
function isHeadingNode(node) {
  return typeof node !== 'string' && isHeadingTag(getElementTag(node));
}
