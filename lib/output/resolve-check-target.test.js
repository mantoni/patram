...
it('defaults check targets to the current project directory', async () => {
  ...
  await expect(resolveCheckTargets([])).resolves.toEqual([
    {
      project_directory,
      target_kind: 'project',
    },
...
it('resolves file and directory targets relative to the project root', async () => {
  ...
  await expect(resolveCheckTargets(['docs/tasks/example.md'])).resolves.toEqual(
    [
      {
        project_directory,
        target_kind: 'file',
  ...
  await expect(resolveCheckTargets(['docs'])).resolves.toEqual([
    {
      project_directory,
...
it('falls back to the target directory when no project config exists above it', async () => {
  ...
  await expect(resolveCheckTargets(['orphan/docs'])).resolves.toEqual([
    {
      project_directory,
      target_kind: 'project',
    },
...
it('filters source files by project, file, and directory targets', () => {
  ...
  expect(
    selectCheckTargetsSourceFiles(source_file_paths, [
      {
...
it('filters diagnostics by project, file, and directory targets', () => {
  ...
  const diagnostics = [
    ...
    createDiagnostic('lib/cli/main.js', 'Example source diagnostic.'),
  ];

  expect(
    selectCheckTargetsDiagnostics(diagnostics, [
      {
        project_directory: '/tmp/project',
        target_kind: 'project',
      },
