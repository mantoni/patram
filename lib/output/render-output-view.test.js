...
it('truncates only the query right title in tty output', () => {
  ...
  expect(
    ...
    'plan docs/plans/v0/query-traversal-and-aggregation.md  [status: acti…]\n' +
...
it('renders rich node descriptions with the same layout as plain output', async () => {
  const output_view = createOutputView(
    'query',
    [
      {
...
it('renders rich query output with the same text layout as plain', async () => {
  ...
  expect(rich_output).toContain(ansis.gray('[status: pending]'));
