 ...
 * @import { BuildGraphResult } from '../graph/build-graph.types.ts';
 */
...
import {
  ...
  createResumeNode,
  createTaskNode,
} from '../graph/reverse-reference-test-helpers.js';
...
it('renders list link summaries inline under the owning list item', async () => {
  project_directory = await mkdtemp(join(tmpdir(), 'patram-show-document-'));

  await writeProjectFile(
    project_directory,
    'docs/patram.md',
    ['# Patram', '', '- Read [guide](./guide.md).'].join('\n'),
  );
  await writeProjectFile(project_directory, 'docs/guide.md', '# Some Guide\n');
...
it('flushes prose footnotes at the end of the current heading section', async () => {
  project_directory = await mkdtemp(join(tmpdir(), 'patram-show-document-'));

  await writeProjectFile(
    project_directory,
    'docs/patram.md',
    createSectionFootnoteSource(),
  );
  await writeProjectFile(project_directory, 'docs/guide.md', '# Some Guide\n');
  ...
  expect(show_output).toEqual({
    ...
    value: {
      incoming_summary: {},
      path: 'docs/patram.md',
      rendered_source: createSectionFootnoteRenderedSource(),
      resolved_links: [
        {
          label: 'guide',
          reference: 1,
...
function createExpectedPromotedShowOutput() {
  return {
    ...
    value: {
      ...
      path: 'docs/patram.md',
      rendered_source:
        '# Patram\n\nSee [decision](./decisions/query-language.md)[^1].\n\n[^1] decision docs/decisions/query-language.md  (status=accepted)\n    Query Language',
      resolved_links: [
        {
          label: 'decision',
          reference: 1,
...
function createSectionFootnoteRenderedSource() {
  return [
    '# Patram',
    '',
    '## Intro',
