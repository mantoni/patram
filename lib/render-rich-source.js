...
function renderInlineNodes(nodes, render_state) {
  ...
  for (const node of nodes) {
    ...
    if (node_tag === 'a') {
      rendered_chunks.push(renderLinkNode(node_children, render_state));
      continue;
    }
 ...
 * @returns {string}
 */
function renderLinkNode(nodes, render_state) {
  const link_text = renderInlineNodes(nodes, render_state);
  const resolved_link =
