...
function formatRichNodeItem(output_item, ansi) {
  const metadata_row = formatRichMetadataRow(output_item, ansi);
  /** @type {string[]} */
  const lines = [ansi.cyan(`document ${output_item.path}`)];
...
function formatRichResolvedLinkItem(output_item, ansi) {
  ...
  /** @type {string[]} */
  const lines = [
    `${ansi.dim(`[${output_item.reference}]`)} ${ansi.cyan(`document ${output_item.target.path}`)}`,
  ];
