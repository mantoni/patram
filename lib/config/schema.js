...
/**
 * @typedef {z.output<typeof class_identity_schema>} ClassIdentityConfig
 */
const class_identity_schema = z.discriminatedUnion('type', [
  z
    .object({
      type: z.literal('document_path'),
    })
    .strict(),
  z
    .object({
      claim_types: z
        .array(z.string().min(1, 'Identity claim types must not be empty.'))
        .min(1, 'Identity claim types must contain at least one claim type.'),
      type: z.literal('claim_value'),
    })
    .strict(),
]);
...
const repo_class_definition_schema = class_definition_schema.extend({
  identity: class_identity_schema.optional(),
