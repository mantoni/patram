...
const stored_query_schema = z
  .object({
    ...
    where: z
      .string()
      .min(1, 'Stored query "where" must not be empty.')
      .optional(),
  })
  .strict()
  .superRefine((stored_query, refinement_context) => {
    if (stored_query.cypher !== undefined || stored_query.where !== undefined) {
      return;
    }

    refinement_context.addIssue({
      code: 'custom',
      message: 'Stored queries require either "cypher" or "where".',
      path: ['where'],
    });
  });
