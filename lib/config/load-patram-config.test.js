...
it('defaults to the current working directory', async () => {
  ...
  expect(
    load_result.config?.queries.pending.where ??
      load_result.config?.queries.pending.cypher,
  ).toBe('$class=task and status=pending');
...
it('accepts stored queries with the structural filename field', async () => {
  test_context.project_directory = await createTempProjectDirectory();
  await writeProjectConfig(
    test_context.project_directory,
    JSON.stringify({
  ...
  expect(
    load_result.config?.queries.readme.where ??
      load_result.config?.queries.readme.cypher,
  ).toBe('$filename=README.md');
  expect(load_result.diagnostics).toEqual([]);
});
  ...
  expect(load_result.diagnostics).toEqual([
    {
      ...
      line: 1,
      message:
        'Invalid config at "queries.pending.where": Unknown field "owner".',
      path: '.patram.json',
    },
  ]);
});
  ...
  expect(load_result.diagnostics).toEqual([
    {
      ...
      line: 1,
      message:
        'Invalid config at "queries.pending.where": Field "status" does not support the "^=" operator.',
      path: '.patram.json',
    },
  ]);
});
