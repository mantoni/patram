 ...
 * @param {MetadataDirectiveRuleConfig | undefined} directive_rule
...
export function checkDirectiveValue(
  ...
  const type_definition =
    directive_rule?.type ?? repo_config.directive_types?.[directive_name];

  if (!type_definition || typeof claim.value !== 'string') {
    return [];
  }
...
function isDirectivePathInClass(
  ...
  const target_kind = mapping_definition?.emit?.target_kind ?? 'document';
