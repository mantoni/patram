...
export function checkDirectiveMetadata(
  ...
  if (
    claims.length === 0 ||
    (repo_config.directive_types === undefined &&
      repo_config.metadata_schemas === undefined)
  ) {
    return [];
  }
...
function collectDocumentDiagnostics(
  ...
  const document_kind = graph.nodes[`doc:${document_path}`]?.kind ?? 'document';
  const schema_definition = repo_config.metadata_schemas?.[document_kind];
  ...
  for (const claim of document_claims) {
    ...
    diagnostics.push(
      ...
      ...checkDirectiveValue(
        ...
        mappings,
        repo_config,
        schema_definition?.directives[claim.name],
        document_entity_keys,
        document_paths,
      ),
    );
...
function collectPresenceDiagnostics(
  ...
  const directive_rule = schema_definition?.directives[directive_name];

  if (
    directive_rule?.presence === 'forbidden' ||
    (directive_rule === undefined &&
      schema_definition?.unknown_directives === 'error')
  ) {
    return [
      createOriginDiagnostic(
        claim,
        'directive.forbidden',
        `Directive "${directive_name}" is forbidden for kind "${document_kind}".`,
      ),
    ];
  }

  if (directive_rule?.multiple === true || directive_count <= 1) {
    return [];
  }
  ...
  return [
    createOriginDiagnostic(
      claim,
      'directive.duplicate',
      `Directive "${directive_name}" must appear at most once for kind "${document_kind}".`,
    ),
  ];
}
    ...
    if (
      ...
      diagnostics.push(
        createDocumentDiagnostic(
          ...
          `Missing required directive "${directive_name}" for kind "${document_kind}".`,
        ),
      );
    }
  }
...
function collectPlacementDiagnostics(
  ...
  return [
    createDocumentDiagnostic(
      ...
      `Document kind "${document_kind}" must be placed in path class "${document_path_class}".`,
    ),
  ];
}
