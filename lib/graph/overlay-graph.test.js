...
it('composes overlay nodes and edges into a queryable graph', () => {
  const graph = overlayGraph(createBaseGraph(), {
    edges: [
      {
        from: 'doc:index.md',
        id: 'edge:assigned_to',
...
it('merges overlay fields onto matching nodes without mutating the base graph', () => {
  const base_graph = createBaseGraph();
...
it('preserves and extends document path aliases for semantic document nodes', () => {
  const graph = overlayGraph(createSemanticBaseGraph(), {
    nodes: [
 ...
 * @returns {BuildGraphResult}
 */
function createSemanticBaseGraph() {
  ...
  const graph = {
    ...
    edges: [],
    nodes: {
      'decision:query': createSemanticDocumentNode(
        'decision:query',
        'decision',
 ...
 * @returns {GraphNode}
 */
function createSemanticDocumentNode(node_id, class_name, path) {
  return {
    $class: class_name,
    $id: node_id,
    $path: path,
