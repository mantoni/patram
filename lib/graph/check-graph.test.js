...
it('validates links_to targets through canonical paths on promoted nodes', () => {
  const diagnostics = checkGraph(createGraphWithPromotedLinkTarget(), [
    'docs/patram.md',
  ]);
  ...
  expect(diagnostics).toEqual([
    {
      ...
      line: 4,
      message:
        'Document link target "docs/decisions/query-language.md" was not found.',
      path: 'docs/patram.md',
    },
  ]);
});
 ...
 * @returns {BuildGraphResult}
 */
function createGraphWithPromotedLinkTarget() {
  return {
    edges: [
      {
        from: 'doc:docs/patram.md',
