...
it('filters graph nodes by path glob', () => {
  const query_result = queryGraph(
    createQueryGraph(),
    '$path*=docs/tasks/**/query-*.md',
    QUERY_REPO_CONFIG,
  );
...
it('filters graph nodes by structural filename', () => {
  const query_result = queryGraph(
    createQueryGraph(),
    '$filename=query.md',
    QUERY_REPO_CONFIG,
...
it('does not match structural filename terms on nodes without a path', () => {
  const query_result = queryGraph(
...
it('filters graph nodes by Cypher exists subqueries', () => {
  const query_result = queryGraph(
    createQueryGraph(),
    'MATCH (n:Document) WHERE EXISTS { MATCH (n)-[:IMPLEMENTS_COMMAND]->(command:Command) WHERE command.id = "command:query" } RETURN n',
    QUERY_REPO_CONFIG,
  );
