...
it('parses a path glob term', () => {
  expect(parseWhereClause('$path*=docs/**/query-*.md')).toEqual({
    expression: {
      kind: 'term',
      term: {
        column: 1,
...
it('reports a missing glob value', () => {
  expect(parseWhereClause('$path*=')).toEqual({
    diagnostic: {
      code: 'query.invalid',
      column: 8,
      level: 'error',
...
it('reports a missing bound glob value', () => {
  expect(
    parseWhereClause('$path*=@glob_pattern', {
      bindings: {},
    }),
  ).toEqual({
    diagnostic: {
      ...
      level: 'error',
      line: 1,
      message: 'Missing query binding "glob_pattern".',
      path: '<query>',
    },
    success: false,
  });
...
it('rejects an empty bound glob value', () => {
  expect(
    parseWhereClause('$path*=@glob_pattern', {
      bindings: {
        glob_pattern: '',
...
it('parses an exact relation-target term with bindings', () => {
  expect(
    parseWhereClause('tracked_in=@document', {
      bindings: {
        document: 'contract:foo',
      },
    }),
  ).toEqual({
    expression: {
...
it('reports invalid grouping, count, and list syntax', () => {
  ...
  expect(parseWhereClause('tracked_in=@document')).toEqual({
    diagnostic: {
      code: 'query.binding_missing',
      column: 12,
      level: 'error',
      line: 1,
      message: 'Missing query binding "document".',
      path: '<query>',
    },
    success: false,
  });
