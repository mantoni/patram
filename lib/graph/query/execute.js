...
function getStructuralFieldValue(graph_node, field_name) {
  const node_path = graph_node.$path ?? graph_node.path;

  return {
    $class: graph_node.$class ?? graph_node.kind,
    $filename: node_path ? posix.basename(node_path) : undefined,
    $id: graph_node.$id ?? graph_node.id,
    $path: node_path,
