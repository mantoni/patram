...
function getNodeFieldValidationError(patram_config, node_class, field_name) {
  if (
    isStructuralFieldName(field_name) ||
...
function inheritPathDescriptions(graph_nodes, document_node_references) {
  ...
  for (const [
    ...
    const description_value = getScalarNodeFieldValue(graph_node?.description);
  ...
  for (const graph_node of graph_nodes.values()) {
    if (graph_node.description !== undefined || !graph_node.$path) {
      continue;
    }

    const description_value = path_descriptions.get(graph_node.$path);

    if (description_value) {
      graph_node.description = description_value;
    }
  }
}
...
function setNodeFieldPriority(
  ...
  const field_priority_key = getFieldPriorityKey(graph_node.id, field_name);
...
function createConflictingPriorityFieldError(
  ...
  const current_value = getScalarNodeFieldValue(graph_node[field_name]);

  if (field_name === 'title') {
    return new Error(
      `Node "${graph_node.id}" has conflicting title values "${current_value}" and "${field_value}".`,
    );
  }
