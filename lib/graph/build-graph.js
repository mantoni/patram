...
export function buildGraph(patram_config, claims) {
  ...
  const document_entity_keys = collectDocumentEntityKeys(patram_config, claims);
  /** @type {Set<string>} */
  ...
  inheritPathDescriptions(graph_nodes, document_node_references);
...
function applyNodeMapping(
  ...
  const field_value = resolveNodeFieldValue(claim);
  ...
  if (node_mapping.field === 'title') {
    setNodeFieldPriority(
      graph_node,
      'title',
      field_value,
      title_priorities,
      getTitleSourcePriority(claim),
    );
    return;
  }

  if (node_mapping.field === 'description') {
    setNodeFieldPriority(
      graph_node,
      'description',
      field_value,
      description_priorities,
      getDescriptionSourcePriority(claim),
    );
    return;
  }
...
function getNodeFieldValidationError(patram_config, node_class, field_name) {
  if (field_name === 'description' || field_name === 'title') {
    return null;
  }
...
function createNode(node_id, class_name, node_key) {
  return {
    identity: {
      class_name,
      id: node_id,
      path: class_name === 'document' ? node_key : undefined,
    },
    key: node_key,
    metadata: {},
  };
 ...
 * @returns {string}
 */
function resolveNodeFieldValue(claim) {
...
function applyFallbackTitles(graph_nodes, title_priorities) {
  for (const graph_node of graph_nodes.values()) {
    ...
    graph_node.metadata.title = fallback_title;
    title_priorities.set(graph_node.identity.id, 0);
  }
}
 ...
 * @param {Map<string, import('./document-node-identity.js').DocumentNodeReference>} document_node_references
 */
function inheritPathDescriptions(graph_nodes, document_node_references) {
  /** @type {Map<string, string>} */
  const path_descriptions = new Map();

  for (const [
    document_path,
    document_node_reference,
  ] of document_node_references) {
    const graph_node = graph_nodes.get(document_node_reference.id);
    const description_value = getScalarNodeFieldValue(
      graph_node?.metadata.description,
    );

    if (description_value) {
      path_descriptions.set(document_path, description_value);
    }
  }

  for (const graph_node of graph_nodes.values()) {
    if (
      graph_node.metadata.description !== undefined ||
      graph_node.identity.path === undefined
    ) {
      continue;
    }

    const description_value = path_descriptions.get(graph_node.identity.path);

    if (description_value) {
      graph_node.metadata.description = description_value;
    }
  }
}
...
function getFallbackTitle(graph_node) {
  ...
  return getNodeIdKey(graph_node.identity.id);
}
...
function setNodeFieldPriority(
  ...
  const field_priority_key = getFieldPriorityKey(
    graph_node.identity.id,
    field_name,
  );
  const current_priority = field_priorities.get(field_priority_key);

  if (current_priority === undefined) {
    graph_node.metadata[field_name] = field_value;
    field_priorities.set(field_priority_key, source_priority);
    return;
  }
  ...
  if (
    ...
    throw createConflictingPriorityFieldError(
      graph_node,
      field_name,
      field_value,
    );
 ...
 * @returns {Error}
 */
function createConflictingPriorityFieldError(
  graph_node,
  field_name,
  field_value,
) {
  const current_value = getScalarNodeFieldValue(
    graph_node.metadata[field_name],
  );

  if (field_name === 'title') {
    return new Error(
      `Node "${graph_node.identity.id}" has conflicting title values "${current_value}" and "${field_value}".`,
    );
  }

  return new Error(
    `Node "${graph_node.identity.id}" has conflicting values for field "${field_name}": "${current_value}" and "${field_value}".`,
  );
}
 ...
 * @returns {number}
 */
function getDescriptionSourcePriority(claim) {
  return claim.type === 'document.description'
 ...
 * @returns {string}
 */
function getFieldPriorityKey(node_id, field_name) {
 ...
 */
function getScalarNodeFieldValue(field_value) {
  if (Array.isArray(field_value)) {
    return field_value.find((value) => typeof value === 'string');
  }
 ...
 * @returns {number}
 */
function getTitleSourcePriority(claim) {
  return claim.type === 'document.title'
...
function setSingleValueField(graph_node, field_name, field_value) {
  const current_value = graph_node.metadata[field_name];
  ...
  if (current_value !== field_value) {
    ...
    throw new Error(
      `Node "${graph_node.identity.id}" has conflicting values for field "${field_name}": "${current_value_text}" and "${field_value}".`,
    );
  }
}
...
function setMultiValueField(graph_node, field_name, field_value) {
  ...
  if (Array.isArray(current_value)) {
    ...
    graph_node.metadata[field_name] = [...current_value, field_value].sort(
      compareFieldValues,
    );
    return;
  }
