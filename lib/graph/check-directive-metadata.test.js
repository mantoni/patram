 ...
 * @import { ClassFieldRuleConfig, MetadataFieldConfig, PatramDiagnostic, PatramRepoConfig } from '../config/load-patram-config.types.ts';
...
it('reports markdown directive validation diagnostics', () => {
  const check_fixture = createCheckFixture({
    ...
    'docs/tasks/task.md': createInvalidMarkdownTaskSource(),
  });
...
function createExpectedJsdocDiagnostics() {
  return [
    createDiagnostic(
      ...
      4,
      'directive.duplicate',
      'Directive "status" must appear at most once for class "document".',
    ),
    createDiagnostic(
      'lib/misc.js',
      7,
...
function createMarkdownDiagnostic(line, column, code, message) {
  return createDiagnostic('docs/tasks/task.md', line, column, code, message);
}
