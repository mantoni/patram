...
it('renders heading colors, inline-code background, and blockquote background in color mode', async () => {
  ...
  expect(rich_source).toContain(ansis.bold.red('# Title'));
  expect(rich_source).toContain(ansis.bold.blueBright('## Subtitle'));
  expect(rich_source).toContain(ansis.bgGray(' quote '));
  expect(rich_source).toContain(ansis.bg(236).dim('code'));
...
it('pads fenced code blocks to one shared box width with one surface color', async () => {
  ...
  expect(ansis.strip(rich_source)).toBe(
    createFencedCodeBlockLines('text [raw.txt]', ['plain', 'wider line']).join(
      '\n',
    ),
  );
  expect(ansis.strip(rich_source)).not.toContain('█');
  expect(rich_source).toContain(ansis.bg(236)(' wider line'));
  expect(rich_source).not.toContain('[48;2;');
});
