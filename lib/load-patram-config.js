...
const directive_type_base_shape = {
  path_class: z.string().min(1).optional(),
};

const directive_type_schema = z.discriminatedUnion('type', [
  z
    .object({
      ...directive_type_base_shape,
  ...
  z
    .object({
      ...
      type: z.literal('enum'),
      values: z
        .array(z.string().min(1, 'Directive enum values must not be empty.'))
        .min(1, 'Directive enum values must contain at least one value.'),
    })
    .strict(),
  z
    .object({
...
const metadata_directive_rule_schema = z
  .object({
    multiple: z.boolean().optional(),
    presence: z.enum(['required', 'optional', 'forbidden']),
    type: directive_type_schema.optional(),
  })
  .strict();
...
const metadata_schema_schema = z
  .object({
    directives: z
      .record(z.string().min(1), metadata_directive_rule_schema)
      .default({}),
    document_path_class: z.string().min(1).optional(),
    unknown_directives: z.enum(['ignore', 'error']).optional(),
  })
  .strict();
...
const patram_repo_config_schema = z
  .object({
    derived_summaries: z
      ...
      .optional(),
    directive_types: z
      .record(z.string().min(1), directive_type_schema)
      .optional(),
    include: z
    ...
    metadata_schemas: z
      .record(z.string().min(1), metadata_schema_schema)
      .optional(),
...
export async function loadPatramConfig(project_directory = process.cwd()) {
  ...
  const directive_validation_diagnostics =
    validateDirectiveValidationConfig(normalized_config);

  if (directive_validation_diagnostics.length > 0) {
    return createLoadResult(null, directive_validation_diagnostics);
  }
 ...
 * @returns {PatramDiagnostic[]}
 */
function validateDirectiveValidationConfig(repo_config) {
  const path_classes = repo_config.path_classes ?? {};
  /** @type {PatramDiagnostic[]} */
  const diagnostics = [];

  collectTopLevelDirectiveTypeDiagnostics(
    diagnostics,
    path_classes,
    repo_config.directive_types,
  );
  collectMetadataSchemaConfigDiagnostics(
    diagnostics,
    path_classes,
    repo_config.metadata_schemas,
  );
...
/**
 * @param {{ derived_summaries?: unknown, directive_types?: unknown, include: string[], kinds?: unknown, mappings?: unknown, metadata_schemas?: unknown, path_classes?: unknown, queries: Record<string, { where: string }>, relations?: unknown }} repo_config
 * @returns {PatramRepoConfig}
 */
function normalizeRepoConfig(repo_config) {
  ...
  assignOptionalRepoConfigField(normalized_config, 'kinds', repo_config.kinds);
  assignOptionalRepoConfigField(
    normalized_config,
    'mappings',
 ...
 * @param {import('./load-patram-config.types.ts').DirectiveTypeConfig} type_definition
 * @param {string} diagnostic_path
 */
function collectDirectiveTypeConfigDiagnostics(
  diagnostics,
  path_classes,
  type_definition,
  diagnostic_path,
) {
  if (type_definition.path_class === undefined) {
    return;
  }
  ...
  if (type_definition.type !== 'path') {
    diagnostics.push(
      createConfigDiagnostic(
        `${diagnostic_path}.path_class`,
        'Path classes are only valid for path directive types.',
      ),
    );
  ...
  if (path_classes[type_definition.path_class]) {
    return;
  }
  ...
  diagnostics.push(
    createConfigDiagnostic(
      `${diagnostic_path}.path_class`,
      `Unknown path class "${type_definition.path_class}".`,
    ),
  );
}
 ...
 * @param {PatramDiagnostic[]} diagnostics
 * @param {Record<string, { prefixes: string[] }>} path_classes
 * @param {PatramRepoConfig['directive_types']} directive_types
 */
function collectTopLevelDirectiveTypeDiagnostics(
  diagnostics,
  path_classes,
  directive_types,
) {
  ...
  for (const [directive_name, type_definition] of Object.entries(
    directive_types,
  )) {
    collectDirectiveTypeConfigDiagnostics(
      diagnostics,
      path_classes,
      type_definition,
      `directive_types.${directive_name}`,
    );
  }
}
 ...
 * @param {PatramDiagnostic[]} diagnostics
 * @param {Record<string, { prefixes: string[] }>} path_classes
 * @param {PatramRepoConfig['metadata_schemas']} metadata_schemas
 */
function collectMetadataSchemaConfigDiagnostics(
  diagnostics,
  path_classes,
  metadata_schemas,
) {
  ...
  for (const [kind_name, schema_definition] of Object.entries(
    metadata_schemas,
  )) {
    collectDocumentPathClassDiagnostic(
      diagnostics,
      path_classes,
      kind_name,
      schema_definition.document_path_class,
    );
    collectDirectiveRuleConfigDiagnostics(
      diagnostics,
      path_classes,
      kind_name,
 ...
 */
function collectDocumentPathClassDiagnostic(
  diagnostics,
  path_classes,
  kind_name,
  document_path_class,
) {
  if (document_path_class === undefined || path_classes[document_path_class]) {
    return;
  }

  diagnostics.push(
    createConfigDiagnostic(
      `metadata_schemas.${kind_name}.document_path_class`,
      `Unknown path class "${document_path_class}".`,
    ),
  );
}
 ...
 * @param {Record<string, { prefixes: string[] }>} path_classes
 * @param {string} kind_name
 * @param {Record<string, { multiple?: boolean, presence: string, type?: import('./load-patram-config.types.ts').DirectiveTypeConfig }>} directives
 */
function collectDirectiveRuleConfigDiagnostics(
  diagnostics,
  path_classes,
  kind_name,
  directives,
) {
  for (const [directive_name, directive_rule] of Object.entries(directives)) {
    if (directive_rule.type) {
      collectDirectiveTypeConfigDiagnostics(
        diagnostics,
        path_classes,
        directive_rule.type,
        `metadata_schemas.${kind_name}.directives.${directive_name}.type`,
      );
    }

    if (
      directive_rule.presence === 'forbidden' &&
      directive_rule.multiple === true
    ) {
      diagnostics.push(
        createConfigDiagnostic(
          `metadata_schemas.${kind_name}.directives.${directive_name}.multiple`,
          'Forbidden directives must not declare "multiple: true".',
        ),
      );
    }
  }
