...
it('parses queries add with a query clause', () => {
  expect(
    ...
    value: {
      kind: 'command',
      color_mode: 'auto',
      command_arguments: ['add', 'test', '--query', 'x=y'],
      command_name: 'queries',
      output_mode: 'default',
    },
  });
...
it('parses queries update with rename, query, and description options', () => {
  expect(
    parseCliArguments([
      'queries',
      'update',
...
it('rejects invalid queries mutation invocations', () => {
  expect(parseCliArguments(['queries', 'add', 'test'])).toEqual({
    ...
    error: {
      code: 'message',
      message: 'Queries add requires "--query <clause>".',
    },
  });
