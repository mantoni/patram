...
it('parses shared output flags before an ad hoc Cypher query', () => {
  expect(
    parseCliArguments([
      '--plain',
      'query',
    ...
    value: {
      kind: 'command',
      color_mode: 'auto',
      command_arguments: ['--cypher', 'MATCH (n:Task) RETURN n'],
      command_name: 'query',
      output_mode: 'plain',
    },
  });
...
it('parses query inspection flags', () => {
  ...
  expect(
    parseCliArguments([
      'query',
      '--cypher',
      'MATCH (n:Plan) WHERE NOT EXISTS { MATCH (task:Task)-[:TRACKED_IN]->(n) } RETURN n',
      '--lint',
    ]),
  ).toEqual({
    success: true,
...
it('parses queries add with a Cypher query', () => {
  expect(
    parseCliArguments([
      'queries',
      'add',
    ...
    value: {
      kind: 'command',
      color_mode: 'auto',
      command_arguments: ['add', 'test', '--cypher', 'MATCH (n) RETURN n'],
      command_name: 'queries',
      output_mode: 'default',
    },
  });
...
it('parses queries update with rename, Cypher, and description options', () => {
  expect(
    parseCliArguments([
      'queries',
      'update',
...
it('parses refs with an optional Cypher query and plain output', () => {
  expect(
    parseCliArguments([
      'refs',
...
it('rejects query without a stored query name or Cypher query', () => {
