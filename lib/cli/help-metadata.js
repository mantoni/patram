...
const COMMAND_DEFINITIONS = {
  ...
  query: {
    ...
    extra_positionals_message:
      'Query accepts "--cypher", "--where", or a stored query name.',
  ...
  queries: {
    allowed_option_names: new Set(['cypher', 'desc', 'name', 'where']),
    examples: [
      'patram queries',
    ...
    missing_usage_lines: [
      'patram queries',
      'patram queries add <name> --where <clause>',
  ...
  refs: {
    allowed_option_names: new Set(['cypher', 'where']),
    examples: [
...
const HELP_TOPIC_DEFINITIONS = {
  'query-language': {
    ...
    operators: [
      ...
      {
        description: 'Glob match for structural path',
        label: '*=',
      },
      {
