...
function hasAdHocQueryCommand(
  ...
  return (
    command_name === 'query' &&
    (parsed_values.cypher !== undefined || parsed_values.where !== undefined) &&
...
function buildAdHocQueryArguments(
  ...
  if (parsed_values.where !== undefined) {
    return command_name === 'query'
      ? ['--where', parsed_values.where]
      : [command_positionals[0], '--where', parsed_values.where];
  }
...
function buildStoredQueryArguments(command_positionals, parsed_values) {
  ...
  appendOptionValue(command_arguments, '--where', parsed_values.where);
...
function findInvalidQueryMode(
  ...
  if (
    ...
    return createMessageParseError(
      'Query accepts at most one of "--cypher" or "--where".',
    );
  }
...
function validateQueriesAddMutation(parsed_values, command_positionals) {
  ...
  if (parsed_values.cypher === undefined && parsed_values.where === undefined) {
    return createMessageParseError(
      'Queries add requires "--cypher <query>" or "--where <clause>".',
    );
  }

  if (parsed_values.cypher !== undefined && parsed_values.where !== undefined) {
    return createMessageParseError(
      'Queries add accepts at most one of "--cypher" or "--where".',
    );
  }
...
function validateQueriesUpdateMutation(parsed_values, command_positionals) {
  ...
  if (parsed_values.cypher !== undefined && parsed_values.where !== undefined) {
    return createMessageParseError(
      'Queries update accepts at most one of "--cypher" or "--where".',
    );
  }
