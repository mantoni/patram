/**
 * @import { ClassFieldRuleConfig, MetadataFieldConfig, PatramRepoConfig } from '../config/load-patram-config.types.ts';
...
async function writeValidFixtureProject(project_directory) {
  await writeProjectFile(project_directory, '.patram.json', '{}\n');
  await writeProjectFile(
    project_directory,
    'docs/decisions/one.md',
    '# Decision\n',
  ...
  await writeProjectFile(project_directory, 'docs/tasks/task.md', '# Task\n');
}
...
async function writeNestedProjectFixtureProject(project_directory) {
  ...
  await writeProjectFile(project_directory, 'nested/.patram.json', '{}\n');
  await writeProjectFile(
    project_directory,
    'nested/docs/tasks/task.md',
    '# Nested Task\n',
